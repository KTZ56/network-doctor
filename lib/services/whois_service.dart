  ...
  Future<WhoisResult> lookup(String domain) async {
    try {
      ...
      final response = await http
          .get(Uri.parse('https://rdap.org/domain/$domain'))
          .timeout(const Duration(seconds: 15));

      if (response.statusCode != 200) {
        return WhoisResult.failure(
          domain: domain,
          message: 'WHOIS server returned ${response.statusCode}.',
        );
      }
      ...
      for (final event in events) {
        if (event['eventAction'] == 'registration') {
          created = event['eventDate'] ?? 'Unknown';
        }

        if (event['eventAction'] == 'expiration') {
          expires = event['eventDate'] ?? 'Unknown';
        }
      }

      final nameServers = ((data['nameservers'] ?? []) as List)
          .map<String>((e) => e['ldhName'].toString())
          .toList();
      ...
      return WhoisResult.failure(
        domain: domain,
        message: 'WHOIS server timed out.',
      );
    } catch (e) {
      return WhoisResult.failure(domain: domain, message: e.toString());
    }
  }
}
